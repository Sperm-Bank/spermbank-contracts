...
contract SpermTreasury is Ownable {
    ...
    event RewardsMinted( address indexed caller, address indexed Frecipient, uint amount );
    ...
    address public immutable Sperm;
    ...
    address public sSperm;
    uint public sSpermQueue; // Delays change to sSperm address
    ...
    constructor (
        ...
        require( _Sperm != address(0) );
        Sperm = _Sperm;

        isReserveToken[ _MIM ] = true;
        reserveTokens.push( _MIM );

    //    isLiquidityToken[ _SpermDAI ] = true;
    //    liquidityTokens.push( _SpermDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for Sperm
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Sperm needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Sperm ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Sperm for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISpermERC20( Sperm ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSperm ).balanceOf( msg.sender ); // Can only borrow against sSperm held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Sperm
        @param _amount uint
     */
    function repayDebtWithSperm( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISpermERC20( Sperm ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Sperm, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Sperm ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Sperm ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Sperm valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Sperm decimals
            value_ = _amount.mul( 10 ** IERC20( Sperm ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSperm ) { // 9
            sSpermQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
